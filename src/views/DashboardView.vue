      ...
      <div class="bg-white border-b border-gray-200 px-8 py-6">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-gray-600 mt-1">Monitor student graduation check-ins in real-time.</p>
          </div>
          <div class="flex items-center gap-3">
            ...
            <button 
              ...
              class="flex items-center gap-2 px-4 py-2.5 rounded-lg bg-indigo-600 text-white font-medium hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition-colors disabled:opacity-50 disabled:cursor-not-allowed shadow-lg shadow-indigo-500/30"
            >
              <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                <path d="M13 2 3 14h9l-1 8 10-12h-9l1-8z"/>
              </svg>
              {{ isSimulating ? 'Simulating...' : 'Simulate RFID Scan' }}
            </button>
          </div>
        </div>
      </div>
      ...
      <div class="p-8">
        ...
        <section class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
          <div class="flex items-center justify-between mb-4">
            ...
            <span v-if="announcing" class="text-sm text-indigo-600 font-medium flex items-center gap-2">
          ...
          <!-- Waiting State -->
          <div v-if="!latestScan" class="flex flex-col items-center justify-center py-12">
            <svg class="w-16 h-16 text-gray-300 animate-spin" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
              <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
              <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
            </svg>
            <p class="mt-4 text-gray-500">Waiting for first scan...</p>
          </div>
          ...
          <div v-else class="flex gap-6 items-start">
            <img 
              ...
              :alt="latestScan.name"
              class="w-28 h-28 rounded-full object-cover border-4 border-indigo-100 shadow-lg"
            />

            <div class="flex-1">
              <h3 class="text-2xl font-bold text-gray-900 uppercase">{{ latestScan.name ?? latestScan.student_name }}</h3>
              <p class="text-indigo-600 font-semibold mt-1 uppercase">{{ latestScan.course }}</p>
              <div class="mt-3 bg-indigo-50 border border-indigo-100 p-4 rounded-lg">
                <p class="text-gray-700 italic">"{{ latestScan.message }}"</p>
              </div>
        ...
        <div v-if="isMonitoring" class="mb-6 flex items-center gap-2 text-sm text-indigo-600 bg-indigo-50 border border-indigo-200 rounded-lg px-4 py-2 w-fit">
        ...
        <section class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          <div class="px-6 py-4 border-b border-gray-200">
            <h2 class="text-lg font-semibold text-gray-900">Recent Scan History</h2>
          </div>

          <!-- Empty State -->
          <div v-if="recentScans.length === 0" class="flex flex-col items-center justify-center py-12">
            ...
            <p class="mt-4 text-gray-500">No scans recorded yet.</p>
          </div>
            ...
            <table class="w-full">
              <thead class="bg-gray-50 border-b border-gray-200">
                <tr>
                  ...
                  <th class="px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">RFID Tag</th>
                </tr>
              </thead>
                ...
                <tr v-for="scan in recentScans" :key="scan.id" class="hover:bg-gray-50 transition-colors">
                  ...
                  <td class="px-6 py-4 whitespace-nowrap">
                    <div class="flex items-center gap-3">
                      ...
                      />
                      <span class="font-medium text-gray-900">{{ scan.student_name }}</span>
                    </div>
                  </td>
                  <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                    {{ scan.course }}
